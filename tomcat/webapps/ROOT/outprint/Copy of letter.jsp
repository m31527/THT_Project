...
<%
	...
	System.out.println(outprintPeopleList.size());
...
<table class="table_sytle">
	...
	<%} %>
		<td WIDTH=50% height="12.5%">
		<div style="overflow:auto">
			<div class="div1" <%if(j==1||j==2){%>style="padding-top: 8px;padding-left:11px;padding-right:11px;"<%}else{%>style="padding-left:11px;padding-right:11px;"<%}%>><%=((People)outprintPeopleList.get(tpc)).getPeople_address()==null?"":((People)outprintPeopleList.get(tpc)).getPeople_address()%></div>
			<div class="div2">&nbsp;&nbsp;&nbsp;&nbsp;<%=((People)outprintPeopleList.get(tpc)).getPeople_name()==null?"":((People)outprintPeopleList.get(tpc)).getPeople_name()%>&nbsp;&nbsp;大德</div>		</div>
		</td>
	<%if(j%2==0){%>	
	</tr>	
	<%} %>
