...
<table align="center">
	<tr>
							...
							if(bean.getPeople_main()==1){
								if(rel_bean.getSame_address()==0){
									...
									<tr>
										...
										<td><button type="button" onclick="location.href='PeopleServlet?action=notJoinPeople&relation_id=<%=rel_bean.getRelation_id()%>&people_id=<%=bean.getPeople_id()%>'">解除</button></td>
									</tr>
									<%
										}
									}		
		...
		<div id="styliz_searchrs" class="searchform" style="margin: 20px auto 0;width: 500px;">
						...
						<%for(Relation rel_bean: bean.getRelation_list()){ 
							if(bean.getPeople_main()==1){
								if(rel_bean.getSame_address()==1){
									%>
									<tr>
										<td><%=rel_bean.getRelation_name() %></td>
										<td><a href="PeopleServlet?action=DetailPeople&people_id=<%=rel_bean.getPeople_id()%>"><%=rel_bean.getPeople_name() %></a></td>
										<td><%=rel_bean.getPeople_birthday() %></td>
										<td><button type="button" onclick="location.href='PeopleServlet?action=notJoinPeople&relation_id=<%=rel_bean.getRelation_id()%>&people_id=<%=bean.getPeople_id()%>'">解除</button></td>
									</tr>
			...
			<!-- 不同地址其他家人 -->
			<div id="stylized" class="myform"  style="margin: 20px auto 0;width: 500px;">
				<form id="insert_search_form" name="insert_search_form" method="post" action="../PeopleServlet?action=QueryByNameForOtherRelation">
				<h1>新增不同地址家人</h1>
					<p>請用搜尋來尋找其他家人</p>
		...
		<td valign="top">
			...
			<div id="stylized" class="myform"  style="margin: 20px auto 0;width: 500px;">
				...
				<input type="text" name="active_create_date" id="active_create_date" value="<%//=MyUtil.getToday("yyyy-MM-dd")%>2011-03-18"/>
